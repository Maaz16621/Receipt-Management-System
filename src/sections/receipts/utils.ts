...
export function applyFilter({
  ...
  if (filterName) {
    ...
    inputData = inputData.filter((receipt) => {
      ...
      const collected_by = receipt.collectedBy && receipt.collectedBy.toLowerCase().includes(lowerCaseFilterName);
      return collected_by || sum_ringgit || received_from || payment_method; // Return true if any match
    });
  }
